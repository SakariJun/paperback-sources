...
var _Sources = (() => {
  ...
  // src/iHentaiVN/iHentaiVN.ts
  var iHentaiVN_exports = {};
  __export(iHentaiVN_exports, {
    iHentaiVN: () => iHentaiVN,
    iHentaiVNInfo: () => iHentaiVNInfo
  });
  ...
  // src/iHentaiVN/iHentaiVNParser.ts
  var import_html_entities = __toESM(require_lib2());
  var iHentaiVNParser = class {
    parseMangaDetails($2, mangaId) {
      let image = $2(".tab-summary .summary_image img").attr("src") ?? "";
      if (!image.startsWith("data:image")) {
        image = encodeURI(image);
      }
      const title = (0, import_html_entities.decode)($2(".post-title h1").first().text().trim());
      const status = "";
      const description = (0, import_html_entities.decode)($2(".description-summary .summary__content p").text().trim());
      const arrayTags = [];
      $2(".summary-content .genres-content a").each((_, element) => {
        const genreName = $2(element).text().trim();
        const path = $2(element).attr("href")?.match(/^https?:\/\/[^\/]+\/(.*?)\/?([?#]|$)/) || null;
        const genreId = path ? path[1] : null;
    ...
    parseChapters($2, mangaId) {
      const chapters = [];
      $2("li.wp-manga-chapter").each((index2, element) => {
        const chapterEl = $2(element).find("a");
        const dateString = $2(element).find(".chapter-release-date i")?.first()?.text();
        const path = chapterEl.attr("href")?.match(`${mangaId}/([^/]+)`);
        const chapterId = path ? path[1] : "";
        const chapterTitle = chapterEl.text().trim();
        const releaseDate = dateString ? this.convertToDate(dateString) : /* @__PURE__ */ new Date();
        const chapNumMatch = chapterTitle.match(/(\d+(\.\d+)?)/i);
    ...
    parseMangaSection($2) {
      const collectedIds = [];
      const itemArray = [];
      $2(".page-item-detail.manga").each((_, element) => {
        const titleElement = $2(element).find(".item-summary .post-title h3 a");
        ...
        const latestChapter = $2(element).find(".chapter-item .chapter a").first().text().trim();
        const path = mangaUrl.match(/truyenhentai\/([^\/?#]+)/);
    ...
    isLastPage($2) {
      return $2("div.nav-previous").length < 1;
    }
    parseImages($2) {
      const images = [];
      $2("img.wp-manga-chapter-img").each((_, element) => {
  ...
  var iHentaiVNInfo = {
    ...
    description: "Extension that pulls manga/hentai from iHentaiVN",
  ...
  var iHentaiVN = class {
    constructor() {
      this.parser = new iHentaiVNParser();
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (homepageSectionId) {
        case "most-views":
          param = `truyenhentai/page/${page}/?m_orderby=views`;
          break;
        case "uncensored":
          param = `hentaiz/truyen-hentai-khong-che/page/${page}`;
          break;
        default:
          param = `truyenhentai/page/${page}`;
      }
    ...
    getMangaShareUrl(mangaId) {
      return `${DOMAIN}/truyenhentai/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DOMAIN}/truyenhentai/${mangaId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.CloudFlareError(response.status);
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DOMAIN}/truyenhentai/${mangaId}/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.CloudFlareError(response.status);
    ...
    async getHomePageSections(sectionCallback) {
      const sections = [
        ...
        {
          request: App.createRequest({
            url: `${DOMAIN}/truyenhentai/?m_orderby=views`,
            method: "GET"
          }),
          sectionID: App.createHomeSection({
            id: "most-views",
        ...
        {
          request: App.createRequest({
            url: `${DOMAIN}/hentaiz/truyen-hentai-khong-che`,
            method: "GET"
          }),
          sectionID: App.createHomeSection({
            id: "uncensored",
      ...
      for (const section of sections) {
        ...
        promises.push(
          this.requestManager.schedule(section.request, 1).then((response) => {
            this.CloudFlareError(response.status);
            const $2 = load(response.data);
            const items = this.parser.parseMangaSection($2);
    ...
    async getSearchResults(query, metadata) {
      ...
      const tags = query?.includedTags?.map((x) => x.id);
      const viewsSort = tags?.includes("views") ?? false;
      const excludedTags = ["views", "new-manga"];
      const request = App.createRequest({
        url: `${DOMAIN}/${tags ? tags?.find(
          (tag) => !excludedTags.includes(tag)
        ) : "truyenhentai"}/page/${page}/?m_orderby=${viewsSort ? "views" : "new-manga"}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
    ...
    async getSearchTags() {
      const tags = [
        { id: "hentaiz/hentaivn", label: "HentaiVN" },
        { id: "hentaiz/hentaihvn", label: "HentaiHVN" },
        { id: "hentaiz/hentaiz", label: "HentaiZ" },
        { id: "hentaiz/hentaiayame", label: "HentaiAyame" },
        { id: "hentaiz/sayhentai", label: "SayHentai" }
      ];
      const sorts = [
        { id: "views", label: "Xem nhi\u1EC1u" },
        { id: "new-manga", label: "M\u1EDBi nh\u1EA5t" }
      ];
      return [
        App.createTagSection({
          id: "web",
...
Please go to the homepage of <${iHentaiVNInfo.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(iHentaiVN_exports);
})();
