...
var _Sources = (() => {
  ...
  var NewTruyen_exports = {};
  __export(NewTruyen_exports, {
    NewTruyen: () => NewTruyen,
    NewTruyenInfo: () => NewTruyenInfo
  });
  ...
  // src/NewTruyen/NewTruyenParser.ts
  var NewTruyenParser = class {
    parseMangaDetails($2, mangaId) {
      let image = $2(".detail-info .avatar img").attr("data-src") ?? $2(".detail-info .avatar img").attr("src") ?? "";
      if (!image.startsWith("data:image")) {
        image = encodeURI(image);
      }
      const title = decode($2("h1.title-detail").first().text().trim());
      const author = decode($2("ul.list-info .author .name")?.next()?.text().trim());
      const status = decode($2("ul.list-info .status .name")?.next()?.text().trim());
      const description = decode($2("#summary").text().trim());
      const arrayTags = [];
      $2(".detail-info a.tr-theloai").each((_, element) => {
    ...
    parseChapters($2, mangaId) {
      const chapters = [];
      $2("nav ul li.row").each((_, element) => {
        ...
        const views = $2(element).find("div.text-center.small").last().text().trim();
    ...
    parseSearchTags($2) {
      const tags = [];
      $2("li.tr-full-truyen a.mb-hover-theloai").each((_, element) => {
        const tagId = $2(element).attr("href")?.split("/tim-truyen/")[1]?.replace(/\//g, "");
        const tagName = $2(element).text().trim();
        if (tagId && tagName && tagId !== "") {
          tags.push({ id: tagId, label: tagName });
        }
      });
      return [
        App.createTagSection({
    ...
    isLastPage($2) {
      return $2("div.next.disabled").length > 0;
    }
    parseImages($2) {
      const images = [];
      $2(".page-chapter").each((_, element) => {
        ...
        const imageUrl = $2(element).find("img[data-index]").attr("src") ?? $2(element).attr("data-src") ?? $2(element).attr("data-lazy-src");
        if (imageUrl) {
    ...
    findNTUrl($2) {
      ...
      for (const el of $2("script").toArray()) {
        const code = $2(el).html();
        if (!code) continue;
  ...
  var NewTruyen = class {
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (homepageSectionId) {
        case "update-recently":
          param = `tim-truyen?sort=0&page=${page}`;
          break;
        case "top-all":
          param = `tim-truyen?sort=10&page=${page}`;
          break;
        case "top-day":
          param = `tim-truyen?sort=13&page=${page}`;
          break;
        case "top-month":
    ...
    async getChapters(mangaId) {
      ...
      if (!response.data || response.data.trim() === "") {
        ...
        const newDomain = this.parser.findNTUrl(load(res.data));
        if (newDomain) {
          this.setNTDomain(newDomain);
          const request2 = App.createRequest({
            url: `${newDomain}/Story/ListChapterByStoryID`,
            method: "POST",
            data: `StoryID=${storyId}`
          });
          response = await this.requestManager.schedule(request2, 1);
        }
      }
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
...
Please go to the homepage of <${NewTruyenInfo.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(NewTruyen_exports);
})();
