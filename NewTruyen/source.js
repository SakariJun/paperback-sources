...
var _Sources = (() => {
  ...
  var NewTruyenParser = class {
    parseMangaDetails($2, mangaId) {
      ...
      const title = (0, import_html_entities.decode)($2("h1.title-detail").first().text().trim());
      const author = (0, import_html_entities.decode)($2("ul.list-info .author .name")?.next()?.text().trim());
      const status = (0, import_html_entities.decode)($2("ul.list-info .status .name")?.next()?.text().trim());
      const description = (0, import_html_entities.decode)($2("#summary").text().trim());
      const arrayTags = [];
