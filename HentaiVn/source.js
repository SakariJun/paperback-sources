...
var _Sources = (() => {
  ...
  var HentaiVnParser = class {
    parseMangaDetails($2, mangaId) {
      let image = $2("#item-detail .detail-info .col-image img").attr("src") ?? "";
      if (!image.startsWith("data:image")) {
        image = encodeURI(image);
      }
      const title = (0, import_html_entities.decode)($2("#item-detail .title-detail").first().text().trim());
      const author = (0, import_html_entities.decode)($2("#item-detail .detail-info .col-info .author p")?.last()?.text().trim());
      const status = (0, import_html_entities.decode)($2("#item-detail .detail-info .col-info .status p")?.last()?.text().trim());
      const description = "";
      const arrayTags = [];
      $2("#item-detail .detail-info .col-info .kind p a").each((_, element) => {
        const genreName = $2(element).text().trim();
        const genreId = $2(element).attr("href")?.split("/tim-truyen/")[1] ?? "";
        if (genreId && genreName) {
          arrayTags.push({ id: genreId, label: genreName });
        }
      });
      const tagSections = [
    ...
    parseChapters($2, mangaId) {
      const chapters = [];
      $2(".list-chapter nav ul li.row").each((_, element) => {
        const chapterElement = $2(element).find("div.chapter a");
        const chapterId = chapterElement.attr("data-id") ?? "";
        const chapterTitle = chapterElement.text().trim();
        const releaseDate = $2(element).find("div.text-center.small").first().text().trim();
        const chapNumMatch = chapterTitle.match(/Chapter (\d+(\.\d+)?)/i);
    ...
    parseChapterDetails($2, mangaId, chapterId) {
      return App.createChapterDetails({
        id: chapterId,
        mangaId,
        pages: this.parseImages($2)
      });
    }
    parseMangaFeaturedSection($2) {
      const collectedIds = [];
      const itemArray = [];
      $2(".items-slide .item").each((_, element) => {
        const titleElement = $2(element).find("h3 a");
        const title = (0, import_html_entities.decode)(titleElement.text().trim());
        const mangaUrl = titleElement.attr("href") ?? "";
        const image = $2(element).find("img").attr("src") ?? "";
        const latestChapter = $2(element).find(".slide-caption a")?.last()?.text().trim();
        const path = mangaUrl.match(/truyen-hentai\/([^\/?#]+)/);
        const mangaId = path ? path[1] : "";
    ...
    parseMangaSection($2, featured = false) {
      if (featured) return this.parseMangaFeaturedSection($2);
      const collectedIds = [];
      const itemArray = [];
      $2(".items .item").each((_, element) => {
        const titleElement = $2(element).find("h3 a");
        const title = (0, import_html_entities.decode)(titleElement.text().trim());
        const mangaUrl = titleElement.attr("href") ?? "";
        const image = $2(element).find(".image img").attr("src") ?? "";
        const latestChapterElement = $2(element).find("li.chapter a").first();
    ...
    parseSearchTags($2) {
      ...
      const sorts = [];
      const minchapters = [];
      $2(".genre-item").each((_, element) => {
        const genreId = $2(element).children("span")?.first()?.attr("data-id");
        const genreName = $2(element).text().trim();
      ...
      return [
        App.createTagSection({
          id: "genres",
          label: "Th\u1EC3 lo\u1EA1i",
          tags: genres.map((genre) => App.createTag(genre))
        }),
        App.createTagSection({
          id: "minchapters",
          label: "S\u1ED1 l\u01B0\u1EE3ng Chap (Ch\u1ECDn 1)",
          tags: minchapters.map((minchapter) => App.createTag(minchapter))
        }),
        App.createTagSection({
          id: "sorts",
          label: "S\u1EAFp x\u1EBFp (Ch\u1ECDn 1)",
          tags: sorts.map((sort) => App.createTag(sort))
        })
      ];
    }
    isLastPage($2) {
      return $2("li.page-item").last().hasClass("disabled");
    }
    parseImages($2) {
      const images = [];
  ...
  var HentaiVnInfo = {
    ...
    description: "Extension that pulls manga/hentai from HentaiVn",
  ...
  var HentaiVn = class {
    constructor() {
      this.parser = new HentaiVnParser();
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (homepageSectionId) {
        case "update-recently":
          param = `tim-truyen?page=${page}`;
          break;
        case "top-all":
          param = `tim-truyen-nang-cao?sort=10&page=${page}`;
          break;
        default:
          param = `tim-truyen?page=${page}`;
      }
      ...
      const manga = this.parser.parseMangaSection($2);
      metadata = this.parser.isLastPage($2) ? void 0 : { page: page + 1 };
    ...
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${DOMAIN}/truyen-hentai/${mangaId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.CloudFlareError(response.status);
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${DOMAIN}/truyen-hentai/${mangaId.replace(/-\d+$/, "")}/chapter/${chapterId}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.CloudFlareError(response.status);
    ...
    async getHomePageSections(sectionCallback) {
      const sections = [
        ...
        {
          request: App.createRequest({
            url: `${DOMAIN}/tim-truyen-nang-cao?sort=10`,
            method: "GET"
          }),
          sectionID: App.createHomeSection({
            id: "top-all",
      ...
      for (const section of sections) {
        ...
        promises.push(
          this.requestManager.schedule(section.request, 1).then((response) => {
            this.CloudFlareError(response.status);
            const $2 = load(response.data);
            const items = this.parser.parseMangaSection($2, section.featured);
    ...
    async getSearchResults(query, metadata) {
      const page = metadata?.page ?? 1;
      const url = `${DOMAIN}/${query.title ? "tim-truyen?" : "tim-truyen-nang-cao"}?page=${page}&keyword=${encodeURI(query.title || "")}`;
      const exGenres = query.excludedTags?.filter((tag) => tag.id.startsWith("genre-")).map((tag) => tag.id.replace("genre-", "")).join(",") ?? "";
      const inGenres = query.includedTags?.filter((tag) => tag.id.startsWith("genre-")).map((tag) => tag.id.replace("genre-", "")).join(",") ?? "";
      const minChapter = query.includedTags?.find((tag) => tag.id.startsWith("minchapter-"))?.id.replace("minchapter-", "") ?? "";
      const sort = query.includedTags?.find((tag) => tag.id.startsWith("sort-"))?.id.replace("sort-", "") ?? "";
      const request = App.createRequest({
        url: `${url}&genres=${inGenres}&notgenres=${exGenres}&minchapter=${minChapter}&sort=${sort}`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
    ...
    async getSearchTags() {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      return this.parser.parseSearchTags($2);
    }
    CloudFlareError(status) {
      if (status == 503 || status == 403) {
        throw new Error(`CLOUDFLARE BYPASS ERROR:
Please go to the homepage of <${HentaiVnInfo.name}> and press the cloud icon.`);
      }
    }
