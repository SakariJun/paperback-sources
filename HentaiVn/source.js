...
var _Sources = (() => {
  ...
  var HentaiVnParser = class {
    parseMangaDetails($2, mangaId) {
      ...
      const title = decode($2("#item-detail .title-detail").first().text().trim());
      const author = decode($2("#item-detail .detail-info .col-info .author p")?.last()?.text().trim());
      const status = decode($2("#item-detail .detail-info .col-info .status p")?.last()?.text().trim());
      const description = "";
      const arrayTags = [];
